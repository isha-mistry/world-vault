...
import { ArrowRight, Wallet } from 'iconoir-react';
...
export const DepositComponent = () => {
  ...
  const handleDeposit = useCallback(async () => {
    ...
    try {
      ...
      const result = await MiniKit.commandsAsync.pay({
        ...
        description: `Deposit ${depositAmount} WLD to Worldcoin Advisor`,
      });
  ...
  }, [depositAmount, session?.user?.id, status]);
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                className="block w-full px-4 py-3 bg-[var(--card)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--primary)] focus:border-[var(--primary)] text-[var(--foreground)] text-lg font-semibold input-focus-fix"
                placeholder="0.5"
                disabled={isProcessing}
                style={{ fontSize: '16px' }}
              />
              <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none">
                <span className="text-[var(--accent)] font-medium text-sm">WLD</span>
              </div>
            </div>
          </div>
